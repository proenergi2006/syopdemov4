 ...
 * field-nya tidak muncul dan form berperilaku seperti semula.
 */
...
const normalizeItem = (item: any): PrItem => {
  ...
  return {
    ...
    /*
     * PR lama (dibuat sebelum fitur ini) tidak punya nilai material group.
     * Dibiarkan null supaya validasi memaksa user memilih saat menyimpan,
     * bukan diam-diam terisi nilai keliru.
     */
    master_material_group_id:
      item.master_material_group_id !== null
      && item.master_material_group_id !== undefined
        ? Number(item.master_material_group_id)
        : null,
...
const loadPurchaseRequestDetail = async (): Promise<void> => {
  if (!publicId.value) {
    loadError.value = t('purchaseRequest.edit.publicIdNotFound')
    return
  }
  ...
  try {
    ...
    form.special_document_type_id = detail.special_document_type_id
      ? Number(detail.special_document_type_id)
      : null
...
const resetItems = async (): Promise<void> => {
  const confirm = await showConfirmAlert({
    title: t('purchaseRequest.edit.toast.resetItemsTitle'),
    text: t('purchaseRequest.edit.toast.resetItemsText'),
...
const selectedVendorTaxDescription = computed(() => {
  ...
  if (isSelectedVendorPkp.value)
    return t('purchaseRequest.edit.taxDescPkp')
...
const validateForm = async (): Promise<boolean> => {
  ...
  if (form.items.some(item => !item.master_material_group_id)) {
    showWarningToast({
      title: t('common.alert.warning'),
...
const updatePurchaseRequest = async (event?: Event): Promise<void> => {
  ...
  const confirm = await showConfirmAlert({
    title: t('purchaseRequest.edit.toast.updateConfirmTitle'),
    text: t('purchaseRequest.edit.toast.updateConfirmText'),
  ...
  try {
    showLoadingAlert(t('purchaseRequest.edit.updatingData'), t('common.alert.pleaseWait'))
    ...
    showErrorToast({
      title: t('common.alert.error'),
      text: err?.response?.data?.message || getApiErrorMessage(error, t('purchaseRequest.create.toast.updateFailed')),
    })
  } finally {
...
const confirmCancel = async (): Promise<void> => {
  const confirm = await showConfirmAlert({
    title: t('purchaseRequest.edit.toast.cancelConfirmTitle'),
    text: t('purchaseRequest.edit.toast.cancelConfirmText'),
      ...
      <div class="d-flex align-center">
        ...
        <div>
          <div class="text-h6 font-weight-medium">
            {{ t('purchaseRequest.edit.loadingTitle') }}
          </div>
    ...
    <VCardText class="pa-6">
      <div class="d-flex align-start justify-space-between flex-wrap gap-4">
        <div class="d-flex align-start">
          ...
          <div>
            ...
            <div class="text-caption text-disabled mt-2">
              {{ t('purchaseRequest.edit.errorHint') }}
            </div>
          </div>
        </div>
        ...
        <div class="d-flex ga-2 flex-wrap">
          ...
          >
            {{ t('purchaseRequest.edit.retryButton') }}
          </VBtn>
          ...
          <VBtn
            ...
            class="text-none"
          >
            {{ t('purchaseRequest.edit.backButton') }}
          </VBtn>
        </div>
      </div>
    </VCardText>
          ...
          <VCardTitle class="d-flex align-center justify-space-between">
            <div>
              <div class="text-h6 font-weight-bold">
                {{ t('purchaseRequest.edit.headerTitle') }}
              </div>
              <div class="text-body-2 text-medium-emphasis">
                        ...
                        <div class="d-flex align-start gap-3">
                          ...
                          <div class="flex-grow-1">
                            ...
                            <div class="text-caption text-medium-emphasis mt-1">
                              {{ t('purchaseRequest.create.items.qtyLabel') }}: <strong>{{ formatDecimalQty(item.qty) || 0 }}</strong>
              ...
              <div class="mt-4">
                <div class="d-flex align-center justify-space-between flex-wrap gap-3 mb-2">
                  <div>
                    ...
                    <div class="text-caption text-medium-emphasis">
                      {{ t('purchaseRequest.edit.attachmentExistingHint') }}
                    </div>
                  </div>
