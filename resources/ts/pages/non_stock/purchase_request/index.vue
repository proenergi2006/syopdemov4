...
const canCancelPurchaseRequest = computed(() => {
  return permissionStore.can('purchase_request.cancel')
})
...
const canExport = computed(() => {
  return permissionStore.can('purchase_request.export')
})
...
const cancelTarget = ref<PurchaseRequestItem | null>(null)
...
const openApprovalHistory = async (
  ...
  if (!item?.public_id) {
    showErrorToast({
      ...
      text: t('purchaseRequest.list.toast.publicIdNotFound'),
    })
...
const approvePurchaseRequest = async (): Promise<void> => {
  ...
  */
  ...
  try {
    showLoadingAlert(
      t('purchaseRequest.list.toast.approveLoading'),
      'Mohon tunggu sebentar',
    )
    ...
    showSuccessToast({
      ...
      text:
        response.data?.message
        || t('purchaseRequest.list.toast.approveSuccessFallback', { nomor: target.nomor_pr || '-' }),
    })
    ...
    */
    ...
    showErrorToast({
      ...
      text: getApiErrorMessage(
        error,
        t('purchaseRequest.list.toast.approveFailedFallback'),
      ),
    })
  }
  finally {
...
const rejectPurchaseRequisition = async (): Promise<void> => {
  ...
  */
  ...
  const confirm = await showConfirmAlert({
    title: t('purchaseRequest.list.toast.rejectConfirmTitle'),
    text: t('purchaseRequest.list.toast.rejectConfirmText', { nomor: target.nomor_pr || '-' }),
    confirmButtonText: t('purchaseRequest.list.toast.rejectConfirmButton'),
  ...
  try {
    showLoadingAlert(
      t('purchaseRequest.list.toast.rejectLoading'),
      'Mohon tunggu sebentar',
    )
    ...
    showSuccessToast({
      ...
      text: response.data?.message
        || t('purchaseRequest.list.toast.rejectSuccessFallback', { nomor: target.nomor_pr || '-' }),
    })
    ...
    */
    ...
    showErrorToast({
      ...
      text: getApiErrorMessage(
        error,
        t('purchaseRequest.list.toast.rejectFailedFallback'),
      ),
    })
  }
  finally {
...
const openCancelPurchaseRequest = (
  row: PurchaseRequestItem,
): void => {
...
const cancelPurchaseRequisition = async (): Promise<void> => {
  if (!cancelTarget.value || cancelLoading.value) return

  if (!cancelNotes.value.trim()) {
    showErrorToast({
      title: t('common.alert.error'),
      text: t('purchaseRequest.list.toast.cancelNotesRequired'),
    })
  ...
  */
  ...
  const confirm = await showConfirmAlert({
    title: t('purchaseRequest.list.toast.cancelConfirmTitle'),
    text: t('purchaseRequest.list.toast.cancelConfirmText', { nomor: target.nomor_pr || '-' }),
    confirmButtonText: t('purchaseRequest.list.toast.cancelConfirmButton'),
    cancelButtonText: t('common.actions.cancel'),
    icon: 'warning',
  })
  ...
  try {
    showLoadingAlert(
      t('purchaseRequest.list.toast.cancelLoading'),
      t('common.alert.pleaseWait'),
    )

    const response = await axios.patch(
      `/transaction/purchase-request/${encodeURIComponent(target.public_id)}/cancel`,
      {
        cancel_notes: notes,
      },
      {
    ...
    showSuccessToast({
      title: t('common.alert.success'),
      text: response.data?.message
        || t('purchaseRequest.list.toast.cancelSuccessFallback', { nomor: target.nomor_pr || '-' }),
    })
    ...
    */
    ...
    showErrorToast({
      ...
      text: getApiErrorMessage(
        error,
        t('purchaseRequest.list.toast.cancelFailedFallback'),
      ),
    })
  }
  finally {
...
const statusItems = computed(() => [
  {
    title: t('purchaseRequest.list.filters.statusAll'),
    value: '',
  },
  {
    title: t('purchaseRequest.list.filters.statusDraft'),
    value: 'DRAFT',
  },
  {
    title: t('purchaseRequest.list.filters.statusInProgress'),
    value: 'IN PROGRESS',
  },
  {
    title: t('purchaseRequest.list.filters.statusApproved'),
    value: 'APPROVED',
  },
  {
    title: t('purchaseRequest.list.filters.statusRejected'),
    value: 'REJECTED',
  },
  {
    title: t('purchaseRequest.list.filters.statusCancelled'),
    value: 'CANCELLED',
  },
])

const statusPOItems = computed(() => [
  {
    title: t('purchaseRequest.list.filters.statusPoAll'),
    value: '',
  },
  {
    title: t('purchaseRequest.list.filters.statusPoOpen'),
    value: 'OPEN',
  },
  {
    title: t('purchaseRequest.list.filters.statusPoPartial'),
    value: 'PARTIAL',
  },
  {
    title: t('purchaseRequest.list.filters.statusPoCompleted'),
    value: 'COMPLETED',
  },
])
...
const getPoLinkStatusColor = (status: string | null): string => {
  const normalized = String(status || '').toUpperCase()
...
const submitPurchaseRequest = async (row: any): Promise<void> => {
  ...
  const confirm = await showConfirmAlert({
    title: t('purchaseRequest.list.toast.submitConfirmTitle'),
    text: t('purchaseRequest.list.toast.submitConfirmText', { nomor: row.nomor_pr }),
    confirmButtonText: t('purchaseRequest.list.toast.submitConfirmButton'),
    cancelButtonText: t('common.actions.cancel'),
  })
  ...
  try {
    showLoadingAlert(t('purchaseRequest.list.toast.submitLoading'), t('common.alert.pleaseWait'))
    ...
    showSuccessToast({
      title: t('common.alert.success'),
      text: response.data?.message || t('purchaseRequest.list.toast.submitSuccessFallback'),
    })
    ...
    showErrorToast({
      title: t('common.alert.error'),
      text: getApiErrorMessage(error, t('purchaseRequest.list.toast.submitFailedFallback')),
    })
  }
}

const goToCreate = (): void => {
  router.push('/non_stock/purchase_request/create')
}
...
| di layar, plus locale aktif agar judul kolom mengikuti bahasa UI.
|
| responseType 'blob' wajib -- tanpa itu axios memperlakukan biner xlsx
| sebagai teks dan file hasil unduhan akan rusak.
|--------------------------------------------------------------------------
*/
...
const exportExcel = async (): Promise<void> => {
  ...
  try {
    showLoadingAlert(
      t('purchaseRequest.list.toast.exportLoading'),
      t('common.alert.pleaseWait'),
    )

    const response = await axios.get('/transaction/purchase-request/export-excel', {
      params: {
    ...
    /*
     * Nama file diambil dari header Content-Disposition agar mengikuti
     * penamaan dari backend (termasuk timestamp-nya).
     */
    ...
    showSuccessToast({
      title: t('common.alert.success'),
      text: t('purchaseRequest.list.toast.exportSuccess'),
    })
  } catch (error: unknown) {
    closeAlert()
    ...
    showErrorToast({
      title: t('common.alert.error'),
      text: t('purchaseRequest.list.toast.exportFailed'),
    })

    console.error('[Purchase Requisition] EXPORT ERROR:', error)
  } finally {
...
const goToEdit = (publicId: string): void => {
  router.push(`/non_stock/purchase_request/edit?id=${publicId}`)
}
...
const openDelete = async (row: any): Promise<void> => {
  if (String(row.status || '').toUpperCase() !== 'DRAFT') {
    showErrorToast({
      title: t('purchaseRequest.list.toast.deleteNotAllowedTitle'),
      text: t('purchaseRequest.list.toast.deleteNotAllowedText'),
    })
...
const getSimpleApprovalStatusLabel = (status: string): string => {
  ...
  if (normalized === 'APPROVED')
    return t('purchaseRequest.detail.approvalStatus.approved')

  if (normalized === 'WAITING')
    return t('purchaseRequest.detail.approvalStatus.waiting')

  if (normalized === 'PENDING')
    return t('purchaseRequest.detail.approvalStatus.pending')

  if (normalized === 'REJECTED')
    return t('purchaseRequest.detail.approvalStatus.rejected')

  if (normalized === 'CANCELLED')
    return t('purchaseRequest.detail.approvalStatus.cancelled')
...
const getDetailValueDifferenceLabel = (detail: any | null | undefined): string => {
  /*
  |--------------------------------------------------------------------------
  | value_difference_label dari backend sengaja tidak dipakai
  |--------------------------------------------------------------------------
  | Backend mengembalikan label dalam Bahasa Indonesia saja, sehingga akan
  | menimpa terjemahan i18n di sini. Label selalu dihitung ulang di FE dari
  | value_difference_type (netral bahasa) supaya ikut berubah sesuai locale.
  |--------------------------------------------------------------------------
  */
  const type = getDetailValueDifferenceType(detail)

  if (type === 'EFFICIENCY')
    return t('purchaseRequest.detail.valueDifferenceEfficiency')

  if (type === 'INCREASE')
    return t('purchaseRequest.detail.valueDifferenceIncrease')

  return t('purchaseRequest.detail.valueDifferenceDefault')
}
  ...
  if (success) {
    ...
    setTimeout(() => {
      if (success === 'created') {
        showSuccessToast({
          title: t('common.alert.success'),
          text: t('purchaseRequest.list.toast.createdSuccess'),
        })
      }
      ...
      if (success === 'updated') {
        showSuccessToast({
          title: t('common.alert.success'),
          text: t('purchaseRequest.list.toast.updatedSuccess'),
        })
      }
    }, 300)
  }
      ...
      <VCardText class="pa-5">
        <div class="d-flex align-center justify-space-between flex-wrap gap-3 mb-5">
          <div class="d-flex align-center gap-3">
            ...
            <div>
              <div class="text-h5 font-weight-bold">
                {{ t('purchaseRequest.list.filtersTitle') }}
              </div>

              <div class="text-body-2 text-medium-emphasis mt-1">
                {{ t('purchaseRequest.list.filtersSubtitle') }}
              </div>
            </div>
          </div>
          ...
          <VBtn
            ...
            @click="resetFilters"
          >
            {{ t('purchaseRequest.list.filters.resetButton') }}
          </VBtn>
        </div>
        ...
        <VRow class="pr-filter-grid">
          ...
          >
            <VTextField
              v-model="searchQuery"
              :label="t('purchaseRequest.list.filters.searchLabel')"
              :placeholder="t('purchaseRequest.list.filters.searchPlaceholder')"
              density="compact"
          ...
          >
            <AppDateTimePicker
              ...
              :label="t('purchaseRequest.list.filters.startDate')"
              density="compact"
              clearable
          ...
          >
            <AppDateTimePicker
              ...
              :label="t('purchaseRequest.list.filters.endDate')"
              density="compact"
              clearable
              :config="{ dateFormat: 'Y-m-d' }"
            />
          ...
          >
            <VSelect
              ...
              :label="t('purchaseRequest.list.filters.statusLabel')"
              :items="statusItems"
              item-title="title"
              item-value="value"
              density="compact"
          ...
          >
            <VSelect
              ...
              :label="t('purchaseRequest.list.table.statusPo')"
              :items="statusPOItems"
              item-title="title"
              item-value="value"
              density="compact"
              ...
              <div class="d-flex align-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  <div class="text-caption text-medium-emphasis approval-filter-subtitle">
                    {{ t('purchaseRequest.list.onlyMyApproval') }}
                  </div>
                </div>
              </div>
    ...
    <VCard>
      <VCardText class="d-flex flex-wrap gap-4 align-center">
        <VBtn
          ...
          @click="goToCreate"
        >
          {{ t('purchaseRequest.list.createButton') }}
        </VBtn>
        ...
        <VBtn
          ...
          @click="exportExcel"
        >
          {{ t('purchaseRequest.list.exportButton') }}
        </VBtn>
        ...
        <div class="d-flex align-center gap-2">
          ...
          <VChip
            ...
            variant="tonal"
          >
            {{ t('purchaseRequest.list.toast.loadingText') }}
          </VChip>
          ...
          >
            {{ t('purchaseRequest.list.toast.reloadData') }}
          </VBtn>
        </div>
      </VCardText>
      ...
      <VTable class="text-no-wrap">
        <thead>
          <tr>
            <th scope="col">{{ t('purchaseRequest.list.table.no') }}</th>
            <th scope="col">{{ t('purchaseRequest.list.table.nomorPr') }}</th>
            <th scope="col">{{ t('purchaseRequest.list.table.tanggal') }}</th>
            <th scope="col">{{ t('purchaseRequest.list.table.cabang') }}</th>
            <th scope="col">{{ t('purchaseRequest.list.table.department') }}</th>
            <th scope="col" class="text-end">{{ t('purchaseRequest.list.table.total') }}</th>
            <th scope="col">{{ t('purchaseRequest.list.table.statusApproval') }}</th>
            <th scope="col">
              <span class="d-inline-flex align-center gap-1">
                {{ t('purchaseRequest.list.table.statusPo') }}
                ...
                <VTooltip
                  ...
                  max-width="320"
                >
                  {{ t('purchaseRequest.list.table.statusPoTooltip') }}
                </VTooltip>
              </span>
            </th>
            <th scope="col">{{ t('purchaseRequest.list.table.nomorPo') }}</th>
            <th scope="col" class="text-center" style="width: 5rem;">{{ t('purchaseRequest.list.table.actions') }}</th>
          </tr>
        </thead>
        ...
        <tbody>
          ...
          >
                ...
                <template #activator="{ props }">
                  ...
                  >
                    ...
                    >
                      ...
                      {{ t('purchaseRequest.list.menu.waitingMyApprovalBadge') }}
                    </VChip>
                  </div>
                </template>
                  ...
                  <VList>
                    ...
                    >
                      ...
                      <VListItemTitle>
                        {{ t('purchaseRequest.list.menu.viewDetail') }}
                      </VListItemTitle>
                    </VListItem>
                    ...
                    >
                      ...
                      <VListItemTitle>
                        {{ t('purchaseRequest.list.menu.approvalHistory') }}
                      </VListItemTitle>
                    </VListItem>
                      ...
                      <VListItemTitle>
                        {{
                          printLoadingId === v.public_id
                            ? t('purchaseRequest.list.menu.printLoading')
                            : t('purchaseRequest.list.menu.print')
                        }}
                      </VListItemTitle>
                    </VListItem>
                    ...
                    >
                      ...
                      <VListItemTitle class="text-success">
                        {{ t('purchaseRequest.list.menu.approve') }}
                      </VListItemTitle>
                    </VListItem>
                    ...
                    >
                      ...
                      <VListItemTitle class="text-error">
                        {{ t('purchaseRequest.list.menu.reject') }}
                      </VListItemTitle>
                    </VListItem>

                    <VListItem
                      v-if="String(v.status).toUpperCase() === 'APPROVED' && String(v.status_po).toUpperCase() === 'OPEN' && canCancelPurchaseRequest"
                      href="javascript:void(0)"
                      @click="openCancelPurchaseRequest(v)"
                    >
                      <template #prepend>
                        <VIcon
                          icon="tabler-ban"
                      ...
                      <VListItemTitle class="text-error">
                        {{ t('purchaseRequest.list.toast.cancelMenuLabel') }}
                      </VListItemTitle>
                    </VListItem>
                    ...
                    >
                      ...
                      <VListItemTitle>{{ t('purchaseRequest.list.menu.submit') }}</VListItemTitle>
                    </VListItem>
                    ...
                    >
                      ...
                      <VListItemTitle>
                        {{ t('purchaseRequest.list.menu.edit') }}
                      </VListItemTitle>
                    </VListItem>
                    ...
                    >
                      ...
                      <VListItemTitle class="text-error">
                        {{ t('purchaseRequest.list.menu.delete') }}
                      </VListItemTitle>
                    </VListItem>
                  </VList>
              </VMenu>
            ...
            <td>
              <template v-if="v.purchase_orders && v.purchase_orders.length">
                <div class="d-flex align-center gap-1">
                  <VChip
                    :color="getPoLinkStatusColor(v.purchase_orders[0].status)"
                    size="small"
                    variant="tonal"
                  >
                    {{ v.purchase_orders[0].nomor_po || '-' }}
                  </VChip>

                  <VMenu
                    v-if="v.purchase_orders.length > 1"
                  ...
                  >
                    <template #activator="{ props }">
                      ...
                      >
                        +{{ v.purchase_orders.length - 1 }}
                      </VChip>
                    </template>

                    <VList density="compact">
                      <VListItem
                        v-for="po in v.purchase_orders"
                        :key="po.public_id"
                      >
                        <template #prepend>
                          <VChip
                            :color="getPoLinkStatusColor(po.status)"
                            size="x-small"
                            variant="flat"
                            class="me-2"
                          >
        ...
        <tfoot v-show="!rows.length && !loading">
          <tr>
            <td colspan="9" class="text-center">
              {{ t('purchaseRequest.list.toast.noDataAvailable') }}
            </td>
          </tr>
        </tfoot>
      </VTable>
      ...
      <VCardText class="d-flex align-center flex-wrap justify-end gap-4 pa-2">
        <div class="d-flex align-center me-3" style="width: 220px;">
          <span class="text-no-wrap me-3">{{ t('purchaseRequest.list.toast.rowsPerPage') }}</span>
    ...
    >
      <VCard class="rounded-lg">
        <VCardItem>
          ...
          <VCardTitle>
            {{ t('purchaseRequest.list.printLanguageDialog.title') }}
          </VCardTitle>

          <VCardSubtitle>
            {{ t('purchaseRequest.list.printLanguageDialog.subtitle') }}
          </VCardSubtitle>
        </VCardItem>
    ...
    >
      <VCard class="pr-detail-card">
        ...
        <VCardText class="pa-6">
          ...
          >
            ...
            <div class="mt-4 text-medium-emphasis">
              {{ t('purchaseRequest.detail.loading') }}
            </div>
          </div>
          ...
          >
            <div class="d-flex align-center justify-space-between w-100">
              ...
              >
                {{ t('purchaseRequest.detail.retryButton') }}
              </VBtn>
            </div>
          </VAlert>
                ...
                >
                  <VCardText>
                    <div class="d-flex align-center justify-space-between flex-wrap gap-3 mb-4">
                      <div>
                        <div class="text-caption text-medium-emphasis">
                          {{ t('purchaseRequest.detail.labelPr') }}
                        </div>
                        <div class="text-h6 font-weight-bold">
                    ...
                    <VRow>
                      <VCol cols="12" md="6">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.cabang') }}</div>
                      ...
                      <VCol cols="12" md="6">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.department') }}</div>
                      ...
                      <VCol cols="12" md="6">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.kategori') }}</div>
                      ...
                      <VCol cols="12" md="6">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.tipe') }}</div>
                      ...
                      <VCol cols="12" md="6">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.createdAt') }}</div>
                          <div class="info-value">
                      ...
                      <VCol cols="12" md="6">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.submittedBy') }}</div>
                          <div class="info-value">
                      ...
                      <VCol cols="12" md="6">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.submittedAt') }}</div>
                          <div class="info-value">
                      ...
                      <VCol cols="12">
                        <div class="info-box">
                          <div class="info-label">{{ t('purchaseRequest.detail.notes') }}</div>
                          <div class="info-value">
                ...
                <VCard class="h-100 rounded-md total-card">
                  <VCardText>
                    <div class="text-caption text-medium-emphasis mb-1">
                      {{ t('purchaseRequest.detail.grandTotalEstimate') }}
                    </div>
                    <div class="text-h5 font-weight-bold mb-4">
                      Rp {{ formatNumberWithoutRp(getDetailGrandTotal(detailPurchaseRequest)) }}
                    </div>

                    <div class="text-caption text-medium-emphasis mb-2 d-inline-flex align-center gap-1">
                      {{ t('purchaseRequest.list.table.statusPo') }}
                    ...
                    <div
                      v-if="detailPurchaseRequest.purchase_orders?.length"
                      class="mb-4"
                    >
                      <div class="d-flex align-center justify-space-between mb-2">
                        <div class="text-caption text-medium-emphasis">
                          {{ t('purchaseRequest.detail.relatedPo') }}
                        </div>
                        ...
                        <VChip
                          ...
                          variant="tonal"
                        >
                          {{ t('purchaseRequest.detail.poCount', { count: detailPurchaseRequest.purchase_orders.length }) }}
                        </VChip>
                      </div>
                      ...
                      <div class="related-po-scroll">
                        <div class="d-flex flex-column gap-2">
                          ...
                          >
                            {{ t('purchaseRequest.detail.showMore') }}
                          </VBtn>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="text-caption text-medium-emphasis mb-2">
                      {{ t('purchaseRequest.detail.recommendedVendor') }}
                    </div>
                        ...
                        <div class="d-flex align-start gap-3">
                          ...
                          <div class="flex-grow-1 min-w-0">
                            ...
                            <div class="d-flex align-center flex-wrap gap-2 mt-2">
                              ...
                              >
                                {{ t('purchaseRequest.detail.recommendedBadge') }}
                              </VChip>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="vendor-payment-grid">
                          <div class="vendor-payment-item">
                            <div class="vendor-payment-label">
                              {{ t('purchaseRequest.detail.paymentType') }}
                            </div>
            ...
            >
              <VCardText>
                <div class="d-flex align-center justify-space-between flex-wrap gap-3 mb-3">
                  <div class="d-flex align-center gap-2">
                    ...
                    <div class="text-subtitle-1 font-weight-bold">
                      {{ t('purchaseRequest.detail.attachmentsTitle') }}
                    </div>
                  </div>
                  ...
                  <VChip
                    ...
                    variant="tonal"
                  >
                    {{ t('purchaseRequest.detail.fileCount', { count: detailPurchaseRequest.attachments?.length || 0 }) }}
                  </VChip>
                </div>
                ...
                >
                  ...
                  >
                    {{ file.original_filename || t('purchaseRequest.detail.attachmentFallbackName') }}
                  </VBtn>
                </div>
                ...
                <VAlert
                  ...
                  density="compact"
                >
                  {{ t('purchaseRequest.detail.noAttachments') }}
                </VAlert>
              </VCardText>
            </VCard>
            ...
            >
              <VCardText>
                <div class="d-flex align-center justify-space-between flex-wrap gap-3 mb-4">
                  <div>
                    <div class="text-subtitle-1 font-weight-bold">
                      {{ t('purchaseRequest.detail.itemsTitle') }}
                    </div>
                  </div>
                  ...
                  >
                    {{ t('purchaseRequest.detail.itemCount', { count: detailPurchaseRequest.items?.length || 0 }) }}
                  </VChip>
                </div>
                  ...
                  <VTable class="detail-item-table">
                    <thead>
                      <tr>
                        ...
                        <th class="text-center col-qty">{{ t('common.itemTable.qty') }}</th>
                        <th class="text-center col-qty">{{ t('common.itemTable.qtyPo') }}</th>
                        <th class="text-center col-outstanding">{{ t('common.itemTable.outstanding') }}</th>
                        <th class="text-end col-money">{{ t('common.itemTable.hargaUnit') }}</th>
                        <th class="text-end col-money">{{ t('purchaseRequest.detail.subtotalPr') }}</th>
                      </tr>
                    </thead>
                    ...
                    <tbody>
                      ...
                      <tr v-if="!detailPurchaseRequest.items?.length">
                        ...
                        >
                          {{ t('purchaseRequest.detail.noItems') }}
                        </td>
                      </tr>
                    </tbody>
                  </VTable>
                    ...
                    >
                      <VCardText class="py-3 px-4">
                        <div class="summary-row">
                          <span>{{ t('purchaseRequest.detail.subtotalItem') }}</span>
                        ...
                        <template v-if="isDetailPurchaseRequestPKP(detailPurchaseRequest)">
                          <div class="summary-row">
                            <span>{{ t('purchaseRequest.detail.dpp') }}</span>
                            <strong>Rp {{ formatNumberWithoutRp(getDetailDpp(detailPurchaseRequest)) }}</strong>
                          </div>

                          <div class="summary-row">
                            <span>{{ t('purchaseRequest.detail.ppn') }}</span>
                        ...
                        <div class="summary-row grand-total">
                          <span>{{ t('purchaseRequest.detail.grandTotalPr') }}</span>
                          <strong>Rp {{ formatNumberWithoutRp(getDetailGrandTotal(detailPurchaseRequest)) }}</strong>
                        </div>

                        <div class="summary-row">
                          <span>{{ t('purchaseRequest.detail.totalPoDone') }}</span>
                        ...
                        <div class="summary-row outstanding">
                          <span>{{ t('purchaseRequest.detail.outstandingPo') }}</span>
                      ...
                      <VCardText class="py-3 px-4">
                        <div class="d-flex align-center justify-space-between flex-wrap gap-2 mb-3">
                          <div class="d-flex align-center gap-2">
                            ...
                            <div>
                              <div class="text-subtitle-2 font-weight-bold">
                                {{ t('purchaseRequest.detail.approvalHistoryTitle') }}
                              </div>
                              <div class="text-caption text-medium-emphasis">
                                {{ t('purchaseRequest.detail.approvalHistorySubtitle') }}
                              </div>
                            </div>
                          </div>
                          ...
                          <VChip
                            ...
                            variant="tonal"
                          >
                            {{ t('purchaseRequest.detail.approvalStageCount', { count: detailApprovalHistories.length }) }}
                          </VChip>
                        </div>
                        ...
                        <VAlert
                          ...
                          density="compact"
                        >
                          {{ t('purchaseRequest.detail.noApprovalHistory') }}
                        </VAlert>
                      </VCardText>
                    </VCard>
                  </VCol>
        ...
        <VCardActions class="justify-end px-6 py-4">
          ...
          >
            {{ t('purchaseRequest.detail.approveButton') }}
          </VBtn>
          ...
          >
            {{ t('purchaseRequest.detail.rejectButton') }}
          </VBtn>
          ...
          <VBtn
            ...
            class="text-none"
          >
            {{ t('purchaseRequest.detail.closeButton') }}
          </VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
      ...
      <VCard>
        <VCardTitle class="d-flex align-center gap-2">
          <VIcon
            ...
            color="error"
          />
          {{ t('purchaseRequest.list.toast.cancelDialogTitle') }}
        </VCardTitle>
        ...
        <VCardText>
          <p class="text-body-2 mb-4">
            {{ t('purchaseRequest.list.toast.cancelDialogText') }}
            <strong>{{ cancelTarget?.nomor_pr || '-' }}</strong>
          </p>
          ...
          <VAlert
            ...
            class="mb-4"
          >
            {{ t('purchaseRequest.list.toast.cancelIrreversibleWarning') }}
          </VAlert>

          <VTextarea
            v-model="cancelNotes"
            :label="t('purchaseRequest.list.toast.cancelNotesLabel')"
            :placeholder="t('purchaseRequest.list.toast.cancelNotesPlaceholder')"
            rows="4"
            auto-grow
            clearable
          ...
          <div class="text-caption text-medium-emphasis mt-2">
            {{ t('purchaseRequest.list.toast.cancelNotesHint') }}
          </div>
        </VCardText>
        ...
        <VCardActions>
          ...
          <VBtn
            ...
            @click="cancelPurchaseRequisition"
          >
            {{ t('purchaseRequest.list.toast.cancelMenuLabel') }}
          </VBtn>
        </VCardActions>
