...
|--------------------------------------------------------------------------
| Identitas pemohon ditampilkan, bukan diisi. Ia dibaca ulang dari akun di
| server saat menyimpan -- layar tidak mengirimkannya sama sekali, supaya
| tidak ada yang bisa mengatasnamakan orang lain lewat permintaan buatan.
|--------------------------------------------------------------------------
*/
...
const canCreate = computed(() => permissionStore.can('business_trip.create'))

const loadingForm = ref(true)
const saving = ref(false)
const isSubmitted = ref(false)

const employee = ref<EmployeeIdentity>({
  employee_name: '',
...
|--------------------------------------------------------------------------
| Diperiksa juga di server; yang di sini hanya supaya kesalahan ketahuan
| sebelum satu perjalanan penuh selesai diketik.
|--------------------------------------------------------------------------
*/
...
const fetchOptions = async (): Promise<void> => {
  ...
  try {
    const response = await axios.get('/business-trip/perdin/options')

    const payload = response.data?.data

    employee.value = {
      employee_name: payload?.employee?.employee_name ?? '',
      department_name: payload?.employee?.department_name ?? null,
      position_name: payload?.employee?.position_name ?? null,
    }

    if (Array.isArray(payload?.timezones) && payload.timezones.length)
...
const save = async (): Promise<void> => {
  ...
  try {
    ...
    await axios.post('/business-trip/perdin', {
...
<template>
  <section>
    <VAlert
      ...
      variant="tonal"
    >
      {{ t('businessTrip.form.noPermission') }}
    </VAlert>
    ...
    <VCard v-else>
      <VCardTitle class="d-flex align-center justify-space-between flex-wrap gap-3">
        <div>
          <div class="text-h6 font-weight-bold">
            {{ t('businessTrip.form.createTitle') }}
          </div>

          <div class="text-body-2 text-medium-emphasis">
            {{ t('businessTrip.form.createSubtitle') }}
          </div>
        </div>
      ...
      <VCardText>
        <!-- IDENTITAS: ditampilkan, tidak diisi -->
        <div class="text-subtitle-2 font-weight-medium mb-1">
          {{ t('businessTrip.form.identity') }}
        </div>

        <div class="text-body-2 text-medium-emphasis mb-3">
          {{ t('businessTrip.form.identityHint') }}
        </div>
