...
|--------------------------------------------------------------------------
*/
class CashAdvanceRealizationApprovalMail extends Mailable implements ShouldQueue
{
    use Queueable;
    ...
    */
    ...
    public function __construct(
        public CashAdvanceRealization $realization,
        ...
        */
        ...
        $this->realization->loadMissing('cashAdvance:id,advance_number');

        $encryptedId = $this->realization->encrypted_id
            ?? Crypt::encryptString((string) $this->realization->id);

        $this->approvalUrl = url(
            '/fund_request/cash_advance_realization?reference=' . urlencode($encryptedId),
        );
    }
    ...
    {
        return new Content(
            view: 'emails.cash_advance_realization_approval',
            with: [
    ...
    private function getSubject(): string
    {
        $subjectKey = match ($this->mode) {
            'final_approved' => 'mail.realisasi.subject.final_approved',
            'rejected' => 'mail.realisasi.subject.rejected',
            'settled' => 'mail.realisasi.subject.settled',
            'received' => 'mail.realisasi.subject.received',
            'receipt_request' => 'mail.realisasi.subject.receipt_request',
            'return_request' => 'mail.realisasi.subject.return_request',
            'reimburse_request' => 'mail.realisasi.subject.reimburse_request',
            default => 'mail.realisasi.subject.default',
        };

        return __($subjectKey, [
            'realization_number' => $this->realization->realization_number ?: '-',
        ]);
    }
}
